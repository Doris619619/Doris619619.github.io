...
function mark(name) {
  if (["cuhk", "sribd", "robomaster", "berkeley", "itso", "douyin", "shengtu"].includes(name)) return `<span class="history-mark history-mark-${name}"><img src="/assets/organizations/${name}.${name === "douyin" ? "jpg" : "png"}" alt="" width="80" height="80" loading="lazy"></span>`;
...
export function homeTimeline(lang) {
  ...
  const rows = [
    ...
    { ...roles.itso, mark: "itso", text: en ? "I test the university’s AI platform and multi-model integrations, checking streaming responses, access permissions, and compatibility across 64 parameter fields, including Cherry Studio integration." : "参与校内 AI 平台与多模型接入测试，检查流式响应、访问权限和 64 个参数字段的兼容性，并测试 Cherry Studio 接入。", links: link(roles.itso.source, en ? "About the platform" : "平台介绍") },
