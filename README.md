...
内容区最大宽度 1040px，阅读区 700px，使用暖白、柔和玫瑰色和细分隔线。网页界面与大开场统一复用 LivePilot-v2 网页的 LiveNest Sans SC 本地字体，文学原文保留衬线字体。首页先呈现分段的个人背景，再展示研究与写作生活。作品按研究、工程与课程实践分组；日期、论文身份、个人工作与成果放在同一阅读区域，GRACE 的录用信息、第 5 作者及结果数字直接可见。标题和明确的论文／详情链接承担跳转，支持独立键盘访问。时间轴桌面展示横向年度导航，手机纵向展开。详细约束和验证记录见 [设计说明](docs/个人站改版.md) 与 [验收记录](docs/改版验收.md)。

既有 `design-qa.md`、`hero-source-audit.md` 及 font/hero 比较脚本是上一版历史记录，不代表新版结构。参考设计归属见 `THIRD_PARTY_NOTICES.md`；没有复制 Innei 的个人内容或图片。

## 社交资料与交互增强

- 首页社交入口、个人介绍、关于页和页脚提供 LinkedIn 链接。抖音创作介绍包含账号 `92689179314`、2026-09-18 突破 1 万粉丝的记录、原始抖音码和里程碑图片，使用原生可展开区域查看抖音码。
- `content/experience.js` 统一维护 RA、ITSO、深圳市大数据研究院实习与 ECE2050 USTF，关于页和轨迹共用；`content/social.js` 维护账号信息。详细资料依据见 [本轮资料记录](docs/资料与动效更新.md)。
- `src/motion.js` 提供本地 Canvas 星光拖尾、点击星芒；鼠标停止后动画自行结束，触屏及减少动态效果时关闭，切出页面暂停。链接有轻量悬停反馈，原生鼠标、选择文字和点击保留。
